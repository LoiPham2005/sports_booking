...
Layout dùng **sidebar trái** giống admin ([staff/layout.tsx](../frontend/src/app/(staff)/staff/layout.tsx)) — logo + role badge (STAFF cam / MANAGER tím), section venue đang trực, nav với icon, demo role switcher ở dưới cùng. Header phải chỉ còn tiêu đề + avatar + logout.

Role:
- **USE_MOCK=true**: đọc từ URL `?role=manager` (demo flow).
- **USE_MOCK=false**: fetch `/staff/memberships`, là MANAGER nếu user có VenueMember role=MANAGER ở ít nhất 1 venue. Hook [use-staff-role.ts](../frontend/src/lib/use-staff-role.ts) cache module-level (1 fetch / tab lifetime) — tránh nhiều page con tự gọi `/staff/memberships` riêng. Trả `StaffRole | undefined` (`undefined` = đang fetch) → page manager-only render skeleton trong lúc loading, chỉ render AccessDenied khi confirmed staff (tránh nháy "Chỉ MANAGER được truy cập"). `withRole(path, role)` preserve URL param `?role=` cho demo mock mode.
...
- `prompt` — prompt dialog dùng chung (thay `window.prompt()`). Mount `<PromptProvider>` cùng cấp với ConfirmProvider trong `<Providers>`. API: `const prompt = usePrompt(); const value = await prompt({ title, description?, placeholder?, defaultValue?, multiline?, required?, maxLength?, confirmText?, cancelText? })`. Trả `string | null` (null khi user huỷ). `multiline=true` → textarea; `required=true` → disable nút xác nhận khi rỗng; có đếm ký tự khi set `maxLength`. Enter để submit (chế độ 1 dòng), Esc / click ngoài để huỷ. Dùng tại: `/admin/disputes` (ghi chú duyệt/từ chối refund), `/admin/venues` (lý do từ chối venue).
