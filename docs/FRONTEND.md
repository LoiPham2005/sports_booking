...
- **List view** mặc định + toggle Map (placeholder OpenStreetMap/Leaflet).
...
### 4.8. Staff portal
- Layout đơn giản — top bar có chữ "STAFF · {venue name}" + avatar + logout, không có sidebar.
- **Today** (`/staff`): danh sách booking trong ngày tại venue được giao, **nút QR scan to ở header**, stats nhanh (đang chơi / sắp đến / đã xong).
- **Schedule** (`/staff/schedule`): xem lịch theo ngày, switch ngày bằng date picker.
- **Booking detail** (`/staff/bookings/[id]`): read-only thông tin, **nút Check-in (mở scanner)** nếu CONFIRMED, nút Đánh dấu hoàn thành nếu CHECKED_IN.
